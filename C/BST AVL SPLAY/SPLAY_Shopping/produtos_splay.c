...
node_product_t* aux_delete_product (node_product_t *node_delete, int flag){
	if (node_delete->left == NULL){ //encontra o nó
		if (flag){
			node_product_t* aux = node_delete->right;
	...
	flag = 1;
	node_delete->left = aux_delete_product(node_delete->left, flag);
	return node_delete;			
}

node_product_t  *delete_product (node_product_t  *root_product, int code_search, node_product_t *father){
	if  (root_product == NULL)	
		return NULL;		
	else if (root_product->code > code_search){
		father = root_product;
		root_product->left = delete_product(root_product->left, code_search, father);
	}else if(root_product->code < code_search){
		father = root_product;
		root_product->right = delete_product(root_product->right, code_search, father);
	}else {
		if ((root_product->left == NULL)&&(root_product->right == NULL)){
			...
			root_product->right = aux_delete_product (aux->right, 0);
			aux->right->father = root_product;
			root_product->left = aux->left;
			aux->left->father = root_product;
			root_product->father = father;
			free (aux);
			return root_product;
		}
