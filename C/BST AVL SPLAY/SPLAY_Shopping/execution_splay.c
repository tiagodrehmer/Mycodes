...
void gnuplote(FILE* saida, node_category_t* root_category){
	if (root_category){
		if (root_category->father)
			fprintf(saida, "%d->%d\n", root_category->code, root_category->father->code);
		if(root_category->left)
			fprintf (saida, "%d->%d\n", root_category->code, root_category->left->code);
		if(root_category->right)
			fprintf (saida, "%d->%d\n", root_category->code, root_category->right->code);
		gnuplote(saida, root_category->left);
		gnuplote(saida, root_category->right);
	}
}

void gnuplote2(FILE* saida, node_product_t* root_product){
	if (root_product){
		if (root_product->father)
			fprintf(saida, "%d->%d\n", root_product->code, root_product->father->code);
		if(root_product->left)
			fprintf (saida, "%d->%d\n", root_product->code, root_product->left->code);
		if(root_product->right)
			fprintf (saida, "%d->%d\n", root_product->code, root_product->right->code);
		gnuplote2(saida, root_product->left);
		gnuplote2(saida, root_product->right);
	}
}
//As funções daqui:
//======================================================================================================================
...
/*Funcao para checar o tipo de separador*/
	...
	while (fscanf (del, "%s\n", linha) != EOF){
		...
		if (qtd_teste == 2){
			if(contains_categ(root_category,code_categ)){
				root_category = delete_category(root_category, code_categ, NULL);
				code_categ = ceiling_categ(root_category, code_categ);
				if (code_categ != -1)
					root_category = splay_category(get_category(root_category, code_categ));
			}
		}
		else if (qtd_teste == 4){
			aux_categ = get_category(root_category, code_categ);
			if ((aux_categ != NULL) && (contains_product(aux_categ->root_product, code_product))){
				root_category = splay_category(aux_categ);
				aux_categ->root_product = delete_product(aux_categ->root_product, code_product, NULL);
				code_product = ceiling_product(aux_categ->root_product, code_product);
				if (code_product != -1)
					aux_categ->root_product = splay_product(get_product(aux_categ->root_product, code_product));
			}
		}
		//else
	...
	while (fscanf (get, "%s\n", linha)!= EOF){
		...
		if (qtd_teste == 2){
			...
			if (aux_categ){
				...
				if (aux_product)
					aux_categ->root_product = splay_product(aux_product);
			}
		}
		//else
...
node_category_t* aplication_floor(FILE *flooor, node_category_t* root_category){
	...
	while (fscanf (flooor, "%s\n", linha)!= EOF){
		...
		if (qtd_teste == 2){
			...
			if (qtd_teste != -1){
				aux_categ = get_category (root_category, qtd_teste);
				root_category = splay_category(aux_categ);
			}
			//if(qtd_teste == -1)
			...
			if (aux_categ){
				...
				if (qtd_teste != -1){
					aux_product = get_product (aux_categ->root_product, qtd_teste);
					aux_categ->root_product = splay_product(aux_product);
				}
			}	//if(qtd_teste == -1)
				//printf("\nO NUMERO %d NAO POSSUI FLOOR\n", code_product);
...
node_category_t* aplication_ceiling(FILE *ceiling, node_category_t* root_category){
	...
	#if defined (_EXECUTION_TIME)
		stop = clock();
		time = (double)(stop - start)* (double)1000/(double)CLOCKS_PER_SEC;
		printf ("%f\n", time);
	#endif
	return root_category;
}
